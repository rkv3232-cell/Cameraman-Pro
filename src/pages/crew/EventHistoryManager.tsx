...
import {
  Star, Plus, Search, Camera, X, ArrowLeft
} from 'lucide-react';
    ...
    <div className="space-y-6 text-[var(--text-primary)]">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-6 border-b border-[var(--border-light)]">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-[var(--text-primary)]">Event History</h1>
            <p className="text-xs text-[var(--text-secondary)]">Track crew work history and client performance reviews</p>
          </div>
        </div>
        {canManage && (
          <button
            onClick={() => setShowAddModal(true)}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-[#c9a227] to-[#f0c040] text-[#0a0f1e] font-bold text-sm shadow-lg shadow-[#c9a227]/30"
          >
            <Plus size={15} /> Add Event Record
          </button>
        )}
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            { l: 'Completed', v: stats.completed, c: 'text-emerald-500 dark:text-emerald-400' },
            { l: 'Upcoming', v: stats.upcoming, c: 'text-blue-500 dark:text-blue-400' },
            { l: 'Ongoing', v: stats.ongoing, c: 'text-amber-500 dark:text-amber-400' },
          ].map(s => (
            <div key={s.l} className="p-3 rounded-xl bg-[var(--surface-base)] border border-[var(--border-light)] text-center">
              <div className={`text-xl font-black ${s.c}`}>{s.v}</div>
              <div className="text-[10px] text-[var(--text-tertiary)] mt-0.5">{s.l}</div>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-3">
          ...
          <select
            ...
            className="px-3 py-2.5 rounded-xl bg-[var(--surface-base)] border border-[var(--border-light)] text-sm text-[var(--text-secondary)] focus:outline-none focus:border-[#c9a227]/50"
          >
        ...
        {loading ? (
          <div className="space-y-3">
            {[1,2,3].map(i => <div key={i} className="h-20 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--border-light)] animate-pulse" />)}
          </div>
        ) : filtered.length === 0 ? (
            ...
            {filtered.map((evt, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="p-4 rounded-2xl bg-[var(--surface-base)] border border-[var(--border-light)] hover:border-[#c9a227]/20 transition-all"
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <h3 className="font-semibold text-[var(--text-primary)] text-sm truncate">{evt.eventName}</h3>
                    ...
                    {evt.rating && (
                      <div className="flex items-center gap-0.5 flex-shrink-0">
                        ...
                        <span className="text-xs text-[var(--text-secondary)] font-medium">{evt.rating}</span>
                      </div>
                    )}
                  </div>
                </motion.div>
...
function EventStatusBadge({ status }: { status: string }) {
  const cfg: Record<string, string> = {
    ...
    upcoming: 'bg-blue-500/10 text-blue-500 dark:text-blue-400',
    ongoing: 'bg-amber-500/10 text-amber-500 dark:text-amber-400',
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-md rounded-3xl bg-[var(--surface-base)] border border-[var(--border-light)] shadow-2xl overflow-hidden"
      >
        <div className="flex items-center justify-between p-5 border-b border-[var(--border-light)]">
          <h2 className="font-bold text-[var(--text-primary)]">Add Event Record</h2>
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-[var(--surface-hover)] text-[var(--text-secondary)] transition-colors">
            <X size={18} />
          </button>
        </div>
        <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-[var(--border-light)] text-[var(--text-secondary)] text-sm hover:bg-[var(--surface-hover)] transition-colors">Cancel</button>
...
const inputCls = "w-full px-3.5 py-2.5 rounded-xl bg-[var(--surface-hover)] border border-[var(--border-light)] text-[var(--text-primary)] placeholder-[var(--text-tertiary)] text-sm focus:outline-none focus:border-[#c9a227]/50 transition-colors";
