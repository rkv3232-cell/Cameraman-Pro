...
import {
  ...
  Edit3, Trash2, MoreVertical, Calendar
} from 'lucide-react';
...
const STATUS_CONFIG: Record<StaffStatus, { label: string; color: string; icon: any }> = {
  active:    { label: 'Active',    color: 'text-emerald-500 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/20', icon: CheckCircle2 },
  busy:      { label: 'Busy',      color: 'text-amber-500 dark:text-amber-400 bg-amber-500/10 border-amber-500/20',     icon: Clock },
  on_event:  { label: 'On Event',  color: 'text-blue-500 dark:text-blue-400 bg-blue-500/10 border-blue-500/20',        icon: Star },
  offline:   { label: 'Offline',   color: 'text-slate-500 dark:text-slate-400 bg-slate-500/10 border-slate-500/20',     icon: WifiOff },
  suspended: { label: 'Suspended', color: 'text-red-500 dark:text-red-400 bg-red-500/10 border-red-500/20',           icon: XCircle },
};
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-6 border-b border-[var(--border-light)]">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">
              Crew Directory & Operations
            </h1>
            <p className="text-sm text-[var(--text-secondary)] mt-0.5">Manage photography crew, roles, verification and status</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 self-start md:self-auto">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-[var(--border-light)] hover:bg-[var(--surface-hover)] text-sm font-medium transition-colors"
          >
            <Clock size={16} />
            Attendance
          </button>
          <button
            onClick={() => navigate('/crew/events')}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-[var(--border-light)] hover:bg-[var(--surface-hover)] text-sm font-medium transition-colors"
          >
            <Calendar size={16} />
            Event History
          ...
          {canManage && (
            <motion.button
              ...
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-[#c9a227] to-[#f0c040] text-[#0a0f1e] font-bold text-sm shadow-lg shadow-[#c9a227]/30 hover:shadow-[#c9a227]/50 transition-shadow"
            >
              <UserPlus size={16} />
              Add Crew
            </motion.button>
      ...
      <div className="space-y-6">
        ...
        {analytics && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {[
              { label: 'Total Crew', value: analytics.total, color: 'from-[#c9a227] to-[#f0c040]', glow: 'shadow-[#c9a227]/10' },
        ...
        <div className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search crew by name, ID, or email..."
              value={search}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h3 className="text-lg font-semibold text-[var(--text-primary)]">No crew members found</h3>
            <p className="text-sm text-[var(--text-secondary)] mt-1">
              {canManage ? 'Click "Add Crew" to get started' : 'No staff available'}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                    ...
                    {menuOpen && (
                      ...
                      >
                        <div className="p-1">
                          ...
                          />
                          <MenuBtn icon={Trash2} label="Delete Crew" onClick={onDelete} danger />
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
